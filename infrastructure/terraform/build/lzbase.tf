...
resource "aws_resourcegroups_group" "Terraform" {
  name        = format("%s%s%s", var.Application, "rgg", var.EnvCode)
...
resource "aws_s3_bucket" "alblogs" {
  bucket_prefix = format("%s-%s-%s", var.Application, var.EnvCode, "alblogs")
  force_destroy = true

  tags = {
    Name      = format("%s-%s-%s", var.Application, var.EnvCode, "alblogs"),
    rtype     = "storage"
    codeblock = "lzbase"
  }
}
